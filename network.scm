...
(define opt-grammar
  `(
    ...
    (method        "integration method (one of rkfe, rk3, rk4a, rk4b, rkhe, rkbs, rkf45, rkv65, rkf78, rkoz, rkdp, crkdp)"
		     (value (required PLATFORM)
			    (predicate 
			     ,(lambda (x) 
				(let ((s (string->symbol (string-downcase x))))
				  (case s
				    ((rkfe rk3 rk4a rk4b rkhe rkbs rkf45 rkck rkoz rkdp rkf45 rkf78 rkv65 crkdp ) s)
    ...
    (let* ((properties (parse-ul-properties group-name properties-sxml))
      ...
      (let* (
             ...
             (network-tmpl  (case (ivp-simulation-method)
                              ((rkhe rkbs rkf45 rkck rkoz rkdp rkf45 rkf78 rkv65 crkdp) "Network.sml.adaptive.tmpl")
                              (else "Network.sml.tmpl")))
             (sim-tmpl      (case (ivp-simulation-method)
                              ((rkhe rkbs rkf45 rkck rkoz rkdp rkf45 rkf78 rkv65 crkdp) "Sim.sml.adaptive.tmpl")
                              (else "Sim.sml.tmpl")))
             (mlb-tmpl      (case (ivp-simulation-method)
                              ((rkhe rkbs rkf45 rkck rkoz rkdp rkf45 rkf78 rkv65) "Sim.mlb.adaptive.tmpl")
                              ((crkdp) "Sim.mlb.crk.tmpl")
                              (else "Sim.mlb.tmpl")))
