...
;(define range-g '(4.5 5.0 6.0 3.0))
;(define range-eta '(0.9 2.0 4.0 2.0))
(define range-g '(4.5))
(define range-eta '(0.9))
...
(for-each
 (lambda (g)
   (for-each
    (lambda (eta)
      (with-output-to-file 
          (string-append (model-name (sprintf "Brunel_network_delta_g~A_eta~A" g eta)) ".xml")
        (lambda ()
          (print-fragments 
           (generate-XML
            (Prelude 
